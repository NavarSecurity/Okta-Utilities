...
class OktaClient:
        ...
        self.session.headers.update(
            {
                ...
                "User-Agent": "okta-selective-restore/0.1.0",
            }
        )
    ...
    def paged_get(self, path: str, *, params: dict[str, Any] | None = None) -> list[Any]:
        ...
        while next_url:
            if first:
                response = self.session.get(next_url, params=params, timeout=self.timeout_seconds)
                first = False
            else:
                response = self.session.get(next_url, timeout=self.timeout_seconds)
            self.request_log.append(RequestLogEntry("GET", response.url, response.status_code, round(response.elapsed.total_seconds(), 3)))
            if response.status_code >= 400:
                try:
                    body = response.json()
                    msg = body.get("errorSummary") or body.get("message") or response.text
    ...
    def request_summary(self) -> dict[str, Any]:
        ...
        return {
            "totalRequests": len(self.request_log),
            "byStatus": by_status,
            "totalElapsedSeconds": round(sum(entry.elapsed_seconds for entry in self.request_log), 3),
        }
